...
with open('synthseg_volumes_cache.pkl', 'rb') as f:
    synth_seg_data = pickle.load(f)

paths = [
    ('WMn', 'WMn'),
    ('Synthesized CSFn', 'CSFnS'),
    # ('Brain-only synthesized CSFn', 'CSFnSB_wmn'),
    # ('CNN segmentation', 'synth_seg'),
    ('CNN segmentation, weighted loss', 'synth_seg_weighted'),
]
...
for _, predicted_path in paths:
    ...
    for subj_path in subjs:
        if 'synth_seg' in predicted_path:
            for structure in synth_seg_data[predicted_path]:
                struct_vol_diffs[structure] += synth_seg_data[predicted_path][structure]
        else:
